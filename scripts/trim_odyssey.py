...
Odyssey 是画布型世界模型，画面通常撑满整屏（canvas 充满），UI 只在边缘极细。
因此这里用更灵敏的 cropdetect（limit 更低、round=2），且当检测结果为整屏时
直接透传转码（不裁），避免无谓的重新编码。与 HappyOyster/PixVerse 的裁剪参数不同。

用法:
    python scripts/trim_odyssey.py
    python scripts/trim_odyssey.py --input outputs/video/od --output outputs/tvideo/od
    python scripts/trim_odyssey.py --crop 2560:1440:0:0
