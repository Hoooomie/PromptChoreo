...
输入: video/ho  (Playwright 直接录制的原始 webm，未裁剪)
输出: tvideo/ho (按 HappyOyster 内容区裁剪后的 mp4)

HappyOyster 播放器上下通常有较暗的 UI 留白，用 cropdetect 自动检测内容包围盒裁剪。
可加 --crop W:H:X:Y 锁定固定裁剪区（测一次后写死最稳）。

用法:
    python scripts/trim_happyoyster.py
    python scripts/trim_happyoyster.py --input video/ho --output tvideo/ho
    python scripts/trim_happyoyster.py --crop 2560:1380:0:30
    python scripts/trim_happyoyster.py --ss 1.5 --to 58      # 可选时间裁剪(秒)
"""
...
CONFIG = {
    ...
    "limit": 30,          # cropdetect 亮度阈值（HappyOyster 顶部有较暗 UI 条）
    "round": 16,          # 裁剪尺寸对齐
